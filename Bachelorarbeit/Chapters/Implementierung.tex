...
welches größer ist als 1 aber kleiner als \(N\).

Für den ungewöhnlichen Fall, dass \(a\) einen nicht-trivialen Teiler mit \(N\) teilt, 
...
Euklids Algorithmus ist in der Lage den größte gemeinsamen Teiler(gcd) zu berechnen.
...
Die gespeicherten Brüche beinhaltet in ihrem Nenner womöglich Teiler der gesuchten Periode, 
welche bei weiteren Berechnungen dabei helfen könnten, die Periode zu finden.

Darauf folgt der einzige Bestandteil des Algorithmus, 
der auf einem Quantencomputer ausgeführt wird.
...
Die nächste Berechnung wird wieder auf einem klassischen Rechnersystem ausgeführt.
...
Im nächsten Schritt wird überprüft ob der Nenner des Bruches die Periode ist.
Dies wird im ersten Durchlauf nur der Fall sein, 
falls die Messung einen der Zustände ergeben hat, 
die wie in Abbildung~\ref{fig:shor_measure} zu der farblich grünen Kategorien gehört.
Sollte es nicht möglich sein, die Periode direkt zu extrahieren, 
wird anhand des Messergebnis eine Nachberechnung angestoßen.
...
dass zu viele klassische Berechnungen anfallen, die in einem vertretbaren Zeitaufwand nicht durchführbar sind.
...
durch die Nachberechnung bestimmt wurde, 
wird überprüft ob die Periode gerade ist und die folgende Bedingung erfüllt ist:
\[a^{\frac{p}{2}} = -1 \mod N\]
Wenn dies der Fall ist kann mit der Periode und 
Euklids Algorithmus mindestens ein nicht-trivialer Faktor von \(N\) berechnet werden~\cite{Shor_1997}:
...
indem \(N\) mit dem gefundenen Faktor dividiert wird.


Im Flussdiagramm und in der Beschreibung, 
wird die Ausführung des Quantenalgorithmus praktisch als ein Abschnitt dargestellt, 
...
ist es möglich, während der klassischen Nachberechnung, 
bereits eine weiteren Durchlauf der Quantenberechnung durchzuführen. 
Allerdings kann es bei diesem Vorgehen dazu kommen, 
dass die erste Messung bereits die korrekte Periode offenbart, 
weswegen eine parallele zweite Messung unnötige Quanten-Ressourcen verschwendet.
