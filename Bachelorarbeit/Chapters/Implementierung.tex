...
dass es sich um eine andere Person handelt. Eine Anfrage bei der University of Montreal selbst war aus Datenschutzgründen erfolglos. 
Als letzte Option wurde der häufigste Co-Autor kontaktiert. 
Dieser ist mittlerweile im Ruhestand, weshalb es möglich ist, dass dies auch für Stéphane Beauregard gilt. 
Nichtsdestotrotz wurde nachgefragt, ob er Stéphane Beauregard meine Kontaktinformationen inklusive des Betreffs zukommen lassen kann.
Bis zur Abgabe dieser Arbeit kam diesbezüglich keine Antwort.
...
wobei Phasenverschiebungen im Bereich von \(e^{\frac{2\pi i}{2^1}}\) bis \(e^{\frac{2\pi i}{2^n}}\) zur Anwendung kommen. 
...
Ab einem bestimmten Punkt werden die gewirkten Phasenverschiebungen vernachlässigbar klein. 
...
Insbesondere ermöglicht die Anwendung von (klassisch) kontrollierten Phasen-Gattern in Kombination mit einer gezielten Auswahl der gewirkten Phasen-Gatter eine effiziente Approximation. 
Diese Methode führen zu einer Reduzierung des gesamten Ressourcenbedarfs im Hinblick auf die Anzahl der benötigten Phasen-Gatter. 
Dadurch kann eine Genauigkeit erhalten bleiben, 
welche weiterhin für ein akzeptables Endergebnis sorgt.

Sei die approximative Quanten-Fourier-Transformation definiert als \(AQFT_m\), 
dann wendet diese maximal \(m\) Phase-Gatter pro Qubit an. 
Der Parameter \(m\) ist der Grad der \(AQFT_m\) und 
definiert bis zu welcher Präzision von \(e^{\frac{2\pi i}{2^m}}\) ein Phasen-Gatter angewendet wird.
Die vollständige Quanten-Fourier-Transformation entspricht demnach \(AQFT_n\).

Die \(AQFT_m\) kann für ein \(m\) mit:
\[m \geq \text{ld}(n+2)\]
eine Präzision erreichen, die der unteren Genauigkeit der exakten Quanten-Fourier-Transformation entspricht~\cite{Barenco_1996}.
Die Wahrscheinlichkeit mit der \(AQFT_m\) ein genaues Ergebnis zu erhalten entspricht~\cite{cheung2004improved}: 
\[P \geq \frac{4}{\pi^2} - \frac{1}{4n}\].

Bezogen auf die Ressourceneffizienz benötigt die \(AQFT_m\) insgesamt \(\Landau(nm)\) vieler Gatter.
Wird \(m\) wie oben definiert, entspricht dies \(\Landau(n~\text{ld}(n))\). 
Im Gegensatz dazu benötigt die reguläre Quanten-Fourier-Transformation \(\Landau(n^2)\) viele Gatter.

Mit der approximativen Quanten-Fourier-Transformation können im Vergleich zur regulären Variante Gatter eingespart werden, 
