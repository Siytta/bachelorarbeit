...
Aus diesem Grund werden zwei Kontroll-Qubits \(\ket{c_1}\) und \(\ket{c_2}\) eingebaut, 
...
Das Zielregister beinhaltet nun also den gewünschten Zustand \(\ket{\Phi(a+b)\mod N}\) für \(a, b < N\).
...
wenn die Kontroll-Qubits \(\ket{c_1}\) und \(\ket{c_2}\) gesetzt sind.

Die restlichen Gatter des Quantenschaltkreises, welche nicht von \(\ket{c_1}\) und \(\ket{c_2}\) kontrolliert sind, 
führen lediglich die Identitätstransformation durch~\cite{beauregard2003circuit}.
Dies liegt an der Bedingung \(b < N\), 
weswegen bereits der modulare Rest im Zielregister steht und somit keine Berechnung notwendig ist.
...
Da die Quanten-Phase-Estimation den Eigenwert einer \textit{unitären} Transformation extrahiert, 
...
\textit{c\_qbits} definiert die Position der beiden Kontroll-Qubits \(\ket{c_1}\) und \(\ket{c_2}\), 
...
Dadurch, dass \textit{b\_qbits} genau so groß definiert wird, 
...
das auf vier Quantenregister \(\ket{c}_1\ket{x}_{n}\ket{b}_{n+1}\ket{0}_1\)
...
welches auf das Register \(\ket{b}_{n+1}\) wirkt.
...
Deswegen muss sich das \(\ket{b}_{n+1}\) Quantenregister auch in der Fourier-Basis befinden.
...
damit der Inhalt des Quantenregister wieder in der Standardbasis ist.
...
Aufgrund der unterliegenden Gatter umfasst das \(\ket{b}_{n+1}\) Register ein zusätzliches Qubit, 
...
Das \(U_a\)-Gatter dient zur Realisierung der Quanten-Phase-Estimation im Shor-Algorithmus, 
...
Sowohl \(\ket{c}_1\) als auch \(\ket{0}_1\) verzeichnen während der Transformation keine Veränderung, 
welche für das Verständnis des \(U_a\)-Gatter relevant ist.

Der Ablauf des \(U_a\)-Gatter lautet wie folgt:
Als erstes wird ein Gatter zur kontrollierten Multiplikation mit \(a\) ausgeführt. 
Dieses Gatter führt, 
ausgehend vom initialen Zustand der beiden Quantenregister, 
die folgende Transformation durch:
\[
...
kann es vernachlässigt werden und wird keiner Swap-Operation unterzogen.
...
nämlich  \textit{a\_bin} und \textit{N\_bin}, welche jeweils \(a\) und \(N\) binärer Form im Listenformat beschreiben,  
...
die Quanten-Phase-Estimation wie in Abbildung~\ref{fig:shor_n_qubit} unter Verwendung des \(U_a\)-Gatters zu bilden.
...
wenn \(x\) Kontroll-Qubits verwendet werden, 
...
Es gibt Quellen, die mehr als \(2n\) \({U_a}^{2^x}\)-Gatter verwendeten~\cite[229]{nielsen_chuang_2010}.
Nach dem Paper von Peter Shor~\cite{Shor_1997} sind jedoch \(2n\) \({U_a}^{2^x}\)-Gatter ausreichend,
...
In der 8. Zeile wird der Eigenvektor gebildet. 
...
In den Zeilen 9 und 10 wird das zugehörige \({U_a}^{2^x}\)-Gatter mit dem passenden \(x\) auf das Register mit dem Eigenvektor angewendet.
Die Funktion \texttt{mod\_exp} berechnet \((a^{2^x})\mod N\).
In der 11. Zeile wird die inverse Quanten-Fourier-Transformation auf die Kontroll-Qubits angewendet.
Da die Funktion \texttt{Shor} Messungen durchführen soll, 
wird in der 12. Zeile definiert, welche Qubits gemessen werden.
In dem Fall werden die Qubits auf den Positionen \textit{c\_qbits} gemessen, 
da diese die Kontroll-Qubits sind.
Anschließend werden in Zeilen 13 und 14 \textit{number\_shots} viele Messungen durchgeführt und 
das Ergebnis wird als Rückgabewert der Funktion zurückgegeben.

Wie bei der Definition des Quantenschaltkreises in Zeil 5 erkennbar, 
...
\includegraphics[scale = 0.75]{FlussdiagramFaktorisierung.png}
\centering
\end{figure}
...
eine Messung den Messwert im klassischen Bit \(c1\) speichert.
Anschließend kontrolliert das klassische Bit \(c1\) die Phasen-Gatter.

Um nach der Messung dasselbe Qubit für \(\ket{q2}_1\) verwendet zu können, 
muss es in einen eindeutigen Zustand versetzt werden, 
in diesem Fall der Zustand \(\ket{0}\).
Aufgrund der zuvor durchgeführten Messung 
kollabiert der Zustand des Qubit zu dem Ergebnis der Messung, 
...
Im Kontext der Quanten-Phase-Estimation wird an dieser Stelle das zugehörige \(U^{2^x}\)-Gatter von \(\ket{q2}_1\) kontrolliert.
...
die jeweils von einem unterschiedlichen klassischen Bits, 
...
Als Ergebnis besitzt der Quantenschaltkreis für den Shor Algorithmus mit der iterative Quanten-Phase-Estimation eine grundlegende andere Struktur 
im Vergleich zur regulären Variante.
Dies resultiert daraus, dass die iterative Quanten-Fourier-Transformation kein eigener Baustein ist sondern schrittweise zwischen 
...
wird die iterative Quanten-Fourier-Transformation für das Most-Significant-Bit angewendet werden.
...
Stattdessen benötigt diese Variante stets \(2n+3\) Qubits, 
...
\begin{figure} [H]
  ...
  \includegraphics[scale = 0.425]{beauregard_iterative_QPE_qiskit.PNG}
  \centering
  \end{figure}
...
da sowohl die Messergebnisse übereinstimmen, 
...
Daraus folgt, dass die approximative Quanten-Fourier-Transformation einen effizienteren Gatterbedarf aufweist, 
...
\begin{figure} [H]
  ...
  \includegraphics[scale = 0.45]{shor_measure_AQFT.PNG}
  \centering
  \end{figure}
