...
Bei der reguläre Quanten-Fourier-Transformation kontrolliert ein bestimmtes Qubit zunächst die Phasen-Gatter auf die übrigen Qubits, 
...
um den Ressourcenbedarf von \(2n\) Kontroll-Qubits auf ein einzelnes zu reduzieren~\cite{Parker_2000}.
...
indem das Messergebnis des \(\ket{q3}_1\) Qubit nach dem Hadamard-Gatter in dem klassische Bit \(c3\) gespeichert wird und 
anschließend das klassische Bit \(c3\) die Phasen-Gatter kontrolliert.

Damit das selbe Qubit anschließend für \(\ket{q2}_1\) verwendet werden kann, 
muss dieses in einen eindeutigen Zustand versetzt werden, 
in diesem Fall der Zustand \(\ket{0}\).
Aufgrund von der Messung, 
befindet sich das Qubit im selben Zustand wie das Messergebnis, 
also entweder \(\ket{0}\) oder \(\ket{1}\).
Im Falle der \(\ket{1}\) wird ein X-Gatter angewendet, welches durch das Messergebnis kontrolliert wird. 
...
Also wäre in Abbildung~\ref{fig:iterative_iQFT} die Kontroll-Operation an der Stelle der \(\ket{q2}\) Bezeichnung.
Im nächsten Schritt werden die Gatter der Quanten-Fourier-Transformation angewendet, 
gefolgt von einer Messung, dessen Ergebnis in ein anderes klassisches Bit gespeichert wird, 
in diesem Fall \(c2\).
Der Ablauf bei \(\ket{q1}_1\) ist identisch wie zuvor, 
nur das die kontrollierten Phasen-Gatter nun von zwei klassische Bits abhängig sind. 
...
Der Quantenschaltkreis für den Shor Algorithmus mit der iterative Quanten-Phase-Estimation sieht anders als die reguläre Variante aus.
Das liegt daran, dass die inverse Quanten-Fourier Transformation kein eigener Baustein ist sondern schrittweise zwischen 
...
def Shor_sequential(a: int, N: int,number_shots: int = 1000, backend: str = "aer_simulator"):
    ...
    for target in k:
        qc.h(0)
        qc.append(U_gate(n,dezToBin(mod_exp(a, 2**(len(k) - target - 1), N),n+1),dezToBin(N,n+1)),[0] + list(ev_qbits))
