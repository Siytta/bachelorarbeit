...
Bei der Semi-klassischen beziehungsweise iterativen Variante ist das anders.
Es ist möglich, das klassische Signal einer Messung zur Kontrolle von Gattern zu verwenden~\cite{Griffiths_1996} 
Also dient das Messergebnis des jeweiligen Qubits als Kontrollbedingung für ein Phasen-Gatter.
...
Die zweite Methode mit der an Ressourcen gespart werden kann, 
betrifft ebenfalls die Quanten-Fourier-Transformation.

Bei dieser Optimierung wird die Einsparung an Ressourcen erreicht, 
indem die Quanten-Fourier-Transformation approximiert wird.

Bekanntlich wendet die reguläre Quanten-Fourier-Transformation und 
die iterative Quanten-Fourier-Transformation (klassisch) kontrollierte Phasen-Gatter an. 
Dabei werden Phasen-Gatter mit einer Phasenverschiebung von \(e^{\frac{2 \pi i}{2^1}}\) bis zu \(e^{\frac{2 \pi i}{2^n}}\) angewendet.
Es existiert also ein exponentielles Wachstum im Nenner des Bruches des gewirkten Phasenverschiebung, 
welche bei jedem weiteren Phasen-Gatter für ein exponentielle Abnahme der gewirkten Phasenverschiebungen sorgt. 
